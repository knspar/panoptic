...
from justserver.daemon.ip_mapper import IPMApper, MokedIPMapper, Resolved, KType
...
class KubernetesNameBase(BaseExtractor):
    ...
    def value(self, connection: Connection, events: list[Event], time: float | None) -> ResolvedIP | None:
        ip = connection.conn[self.index][0]
        res = ip_mapper.map_ip(ip)
        if res is None:
            return ResolvedIP(name=None, type=None)
        return cast(ResolvedIP, res)
